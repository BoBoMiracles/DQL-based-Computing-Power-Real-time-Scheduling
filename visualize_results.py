...
rates = (1,)
lens = (3, 5, 10)
time = 3600  # 仿真时间固定为3600秒
FOLDER_MAPPING = {}  # 将在主函数中填充
results_folder = ''  # 将在主函数中填充
...
def plot_training_history(model_type, rate, lstm):
    """绘制训练历史（奖励和损失）"""
    # 加载训练历史
    ...
    # 创建结果目录
    os.makedirs(f"{results_folder}/training_history", exist_ok=True)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{results_folder}/training_history/{model_type}_rewards.png", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{results_folder}/training_history/{model_type}_losses_moving_avg.png", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{results_folder}/training_history/{model_type}_losses_filtered_moving_avg.png", dpi=300)
    plt.close()

def compare_models(rate, lstm, sim_time=time):
    """比较不同模型的性能"""
    # 加载训练历史
    ...
    # 创建结果目录
    os.makedirs(f"{results_folder}/model_comparison", exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/reward_comparison_highlight.png", dpi=300)
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/moving_avg_comparison.png", dpi=300)
    plt.close()
    
    # 加载最终模型进行性能测试
    env = ComputingNetworkSimulator('gurobi_solution_service_sources.csv', 'gurobi_solution_compute_nodes.csv', rate=rate, simulation_time=sim_time)
    
    # 测试GNN模型
    gnn_agent = GNNAgent(env, device='cuda')
    gnn_agent.policy_net.load_state_dict(
        torch.load(os.path.join(get_model_path('gnn'), "gnn_dqn_final.pth"), map_location='cuda')
    )
    gnn_metrics = test_model(gnn_agent, env)
    
    # 测试GNN+LSTM模型
    gnn_lstm_agent = LSTMDQNAgent(env, device='cuda')
    gnn_lstm_agent.policy_net.load_state_dict(
        torch.load(os.path.join(get_model_path('gnn_lstm'), "gnn_lstm_dqn_final.pth"), map_location='cuda')
    )
    ...
    metrics_df = pd.DataFrame({
        ...
        'Avg Latency (ms)': [
            gnn_metrics['total_latency'] / gnn_metrics['succeed_requests'] if gnn_metrics['succeed_requests'] > 0 else 0,
            gnn_lstm_metrics['total_latency'] / gnn_lstm_metrics['succeed_requests'] if gnn_lstm_metrics['succeed_requests'] > 0 else 0,
            random_metrics['total_latency'] / random_metrics['succeed_requests'] if random_metrics['succeed_requests'] > 0 else 0
        ],
        'Cloud Usage (%)': [
    ...
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/performance_comparison.png", dpi=300)
    plt.close()
    ...
    plt.ylabel("Reward")
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/avg_reward_comparison.png", dpi=300)
    plt.close()
    
    # 保存性能数据
    metrics_df.to_csv(f"{results_folder}/model_comparison/performance_metrics.csv", index=False)
...
def test_model(agent, env, num_episodes=10):
    ...
    failed_count = num_episodes * metrics['total_requests'] - metrics['success_count']
    if failed_count > 0:
        metrics['avg_failed_latency'] = metrics['failed_latency'] / failed_count
    else:
...
def visualize_request_processing(model_type, rate, lstm):
    """可视化请求处理过程"""
    # 创建结果目录
    os.makedirs(f"{results_folder}/request_processing/{model_type}", exist_ok=True)
    
    # 初始化环境
    env = ComputingNetworkSimulator('gurobi_solution_service_sources.csv', 'gurobi_solution_compute_nodes.csv', rate=rate, simulation_time=time)
    ...
    df.to_csv(f"{results_folder}/request_processing/{model_type}/request_processing.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(f"{results_folder}/request_processing/{model_type}/request_analysis.png", dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{results_folder}/request_processing/{model_type}/latency_vs_demand.png", dpi=300)
    plt.close()

def visualize_resource_utilization(model_type, rate, lstm):
    """可视化资源利用率"""
    # 创建结果目录
    os.makedirs(f"{results_folder}/resource_utilization/{model_type}", exist_ok=True)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{results_folder}/resource_utilization/{model_type}/room_utilization.png", dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{results_folder}/resource_utilization/{model_type}/cloud_usage.png", dpi=300)
    plt.close()
    
    # 保存资源利用率数据
    util_df = pd.DataFrame(room_utilizations)
    util_df['cloud_usage'] = cloud_usage
    util_df.to_csv(f"{results_folder}/resource_utilization/{model_type}/resource_utilization.csv", index=False)

def compare_requests_performance(rate, lstm):
    """比较两个模型在同一批请求序列上的性能表现"""
    # 创建结果目录
    os.makedirs(f"{results_folder}/model_comparison/per_request", exist_ok=True)
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/per_request/request_reward_highlight.png", dpi=300)
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/per_request/request_reward_moving_avg.png", dpi=300)
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/per_request/request_latency_highlight.png", dpi=300)
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/per_request/request_latency_moving_avg.png", dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/per_request/request_position_comparison.png", dpi=300)
    plt.close()
    ...
    plt.ylabel("Count")
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/per_request/request_type_distribution.png", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{results_folder}/model_comparison/per_request/success_rate_by_type.png", dpi=300)
    plt.close()
    
    # 保存数据
    df_gnn.to_csv(f"{results_folder}/model_comparison/per_request/gnn_request_performance.csv", index=False)
    df_gnn_lstm.to_csv(f"{results_folder}/model_comparison/per_request/gnn_lstm_request_performance.csv", index=False)
    pd.DataFrame(requests).to_csv(f"{results_folder}/model_comparison/per_request/request_sequence.csv", index=False)
...
def test_performance_on_requests(env, agent, requests, agent_name):
    ...
    # 重置环境
    state = env.reset()
    env.simulation_time = 10000  # 延长时间避免提前结束
...
def main():
    """主函数：执行所有可视化任务"""
    global results_folder, FOLDER_MAPPING
    ...
    for rate in rates:
        for lstm in lens:
            # 设置文件夹映射和结果目录
            results_folder = f'lstm_rate_results/results_lstm{lstm}_rate{rate}'
            FOLDER_MAPPING = {
                'gnn': f'models/gnn_model_rate{rate}',
                'gnn_lstm': f'models/gnn_lstm{lstm}_model_rate{rate}'
            }

            os.makedirs(results_folder, exist_ok=True)
            print(f"Visualizing models with time_window={lstm} and rate={rate}")
            
            print("Visualizing training history...")
            plot_training_history('gnn', rate, lstm)
            plot_training_history('gnn_lstm', rate, lstm)
            
            print("Comparing models...")
            compare_models(rate, lstm, time)
            
            print("Comparing per-request performance...")
            compare_requests_performance(rate, lstm)
            
            print("Visualizing request processing...")
            visualize_request_processing('gnn', rate, lstm)
            visualize_request_processing('gnn_lstm', rate, lstm)
            
            print("Visualizing resource utilization...")
            visualize_resource_utilization('gnn', rate, lstm)
            visualize_resource_utilization('gnn_lstm', rate, lstm)
