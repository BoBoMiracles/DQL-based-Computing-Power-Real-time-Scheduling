...
class LSTMDQNAgent:
    ...
    def __init__(self, env, device='cuda', history_len=5):
        ...
        # 构建动作列表（云端+所有机房）- 按排序顺序
        room_ids = sorted(env.nodes['rooms'].keys())  # 关键修改：排序机房ID
        self.action_list = ['cloud'] + room_ids
    ...
    def get_action(self, state, epsilon=0.1):
        ...
        if len(self.state_history) < self.history_len:
            # 关键修改：直接使用模拟器提供的合法动作列表
            valid_actions = self.env.get_valid_actions()
            return random.choice(valid_actions) if valid_actions else 'cloud'
    ...
    def remember(self, state, action, reward, next_state, done):
        ...
        if action == 'cloud':
            ...
            # 查找机房在排序列表中的位置
            room_ids = sorted(self.env.nodes['rooms'].keys())
            action_idx = room_ids.index(action) + 1  # +1因为索引0是云端
