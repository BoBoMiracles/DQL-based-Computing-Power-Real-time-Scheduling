...
# 导入新的 Transformer 模型
from gnn_transformer_model import GNNTransformerPolicy
from torch_geometric.data import Batch
from torch_geometric.data import Data
import torch.nn.functional as F

# StateTransformer 和 SequenceReplayBuffer 保持不变
...
class GNNTransformerDDQNAgent:
    """使用 Transformer 替代 LSTM 的 DDQN 智能体"""
    def __init__(self, env, sequence_length, device='cuda', update_type='soft', tau=0.01, target_update_freq=500):
        ...
        # 策略网络和目标网络
        # 实例化新的 GNNTransformerPolicy
        self.policy_net = GNNTransformerPolicy(
            sequence_length=sequence_length,
            action_space_size=self.action_space_size,
            req_feat_dim=5,
            # (您可以调整 Transformer 的特定参数)
            trans_hidden_dim=128, 
            nhead=4, 
            num_encoder_layers=2
        ).to(device)
        
        self.target_net = GNNTransformerPolicy(
            sequence_length=sequence_length,
            action_space_size=self.action_space_size, 
            req_feat_dim=5,
            trans_hidden_dim=128, 
    ...
    def get_action(self, state, epsilon=0.1):
        ...
        with torch.no_grad():
            ...
            # ****** 修改点 ******
            # 移除 hidden_state
            q_values_flat, _ = self.policy_net(batch_data, batch_size_override=1) 
            # ********************
    ...
    def update_model(self):
        ...
        # ****** 修改点 ******
        current_q_flat, _ = self.policy_net(state_batch, batch_size_override=self.batch_size)
        # ********************
        ...
        with torch.no_grad():
            ...
            # ****** 修改点 ******
            next_q_target_flat, _ = self.target_net(next_state_batch, batch_size_override=self.batch_size)
            # ********************
        ...
        with torch.no_grad():
            ...
            # ****** 修改点 ******
            next_q_policy_flat, _ = self.policy_net(next_state_batch, batch_size_override=self.batch_size)
            # ********************
